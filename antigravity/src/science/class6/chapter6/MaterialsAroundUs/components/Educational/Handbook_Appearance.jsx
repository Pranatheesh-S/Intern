      ...
      {bookPage === 1 ? (
        <div style={{ flex: 1, minHeight: 0, padding: '24px 20px', position: 'relative', display: 'flex', flexDirection: 'column', overflowY: 'auto', gap: '16px' }}>
          ...
          <div style={{ background: '#f8fafc', border: '1px dashed #93c5fd', borderRadius: '12px', padding: '16px', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
            ...
            <div style={{ fontSize: 'var(--text-base)', color: '#1e293b', lineHeight: '1.6', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>Materials can look different from each other.</div>
              <div>Some have shiny surfaces, while others look dull.</div>
              <div>They may also differ in <strong style={{ color: '#2563eb' }}>colour</strong> and <strong style={{ color: '#2563eb' }}>texture</strong>, such as smooth or rough.</div>
            </div>
          </div>
          ...
          <div style={{ background: '#faf5ff', border: '1px solid #d8b4fe', borderRadius: '12px', padding: '16px', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
            <div style={{ fontSize: 'var(--text-2xl)', marginTop: '4px' }}>✨</div>
            <div style={{ fontSize: 'var(--text-base)', color: '#1e293b', lineHeight: '1.6', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <strong style={{ color: '#7e22ce', fontSize: 'var(--text-base)' }}>Lustrous vs Non-lustrous</strong>
              <div>Materials with shiny surfaces are called <strong style={{ color: '#7e22ce' }}>lustrous</strong>. Metals like iron, copper and aluminium are usually lustrous.</div>
              <div>Paper, wood and rubber are examples of <strong style={{ color: '#7e22ce' }}>non-lustrous</strong> materials.</div>
            </div>
          </div>
          ...
          <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '12px', padding: '16px', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
            ...
            <div style={{ fontSize: 'var(--text-base)', color: '#1e293b', lineHeight: '1.5' }}>
          ...
          <div style={{ marginTop: 'auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '10px' }}>
            <div style={{ color: '#94a3b8', fontSize: 'var(--text-sm)' }}>Page 1</div>
            <button 
        ...
        <div style={{ flex: 1, minHeight: 0, padding: '24px 20px', position: 'relative', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
          ...
          <div style={{ border: '2px solid #22c55e', borderRadius: '12px', padding: '16px', background: '#f0fdf4', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: 'var(--text-sm)', color: '#166534' }}>
                {isPhase1Done ? <CheckSquare size={18} color="#16a34a" /> : <Square size={18} color="#94a3b8" />} Observe the materials
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: 'var(--text-sm)', color: '#166534' }}>
                {isPhase2Done ? <CheckSquare size={18} color="#16a34a" /> : <Square size={18} color="#94a3b8" />} Group by appearance
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: 'var(--text-sm)', color: '#166534' }}>
                {isPhase3Done ? <CheckSquare size={18} color="#16a34a" /> : <Square size={18} color="#94a3b8" />} Test with the lamp
              </div>
            </div>
          </div>
