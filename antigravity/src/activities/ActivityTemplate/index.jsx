...
export default function ActivityTemplate({ onBackToDashboard }) {
  ...
  const handleStage1Complete = () => {
    ...
    // IMPORTANT RULE: Avoid automatic tab transitions on timeout or passive completion.
    // Instead, your Stage components should render a manual "Next Section" button when finished.
    // That manual button should call `onComplete()`, which then triggers this transition.
    setActiveTab('test');
  };

  const handleStage2Complete = () => {
    setProgress(prev => ({ ...prev, test: true }));
    // Transition triggered manually by a Next button in Stage2
    setActiveTab('explore');
  };

  const handleStage3Complete = () => {
    setProgress(prev => ({ ...prev, explore: true }));
    // Transition triggered manually by a Next button in Stage3
    setActiveTab('quiz');
  };
      ...
      <div className="lab-internal-header" style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <div>
            ...
            <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Customizable Starter Kit</span>
          </div>
        </div>
