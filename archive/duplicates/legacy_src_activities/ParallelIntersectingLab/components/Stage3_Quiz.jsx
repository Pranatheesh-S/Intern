...
export default function Stage3_Quiz({ onComplete, addXp, onBackToDashboard }) {
  ...
  const questions = [
    {
      text: "How many points do two intersecting lines share in common?",
      options: [
        "Zero points",
        "Exactly one point",
        "Two points",
      ...
      correct: 1,
      feedback: "Correct! Two intersecting lines cross at exactly one unique junction point (the point of intersection)."
    },
    {
      text: "Which of the following is a real-world example of parallel lines?",
      options: [
        "The letter 'X' crossings",
        "Opposite edges of a wooden ruler",
        "Hands of a clock at 12:15",
        "Adjacent pages of a closed book"
      ],
      correct: 1,
      feedback: "Correct! The opposite edges of a ruler are perfectly straight, maintain a uniform width, and never meet, representing parallel lines."
    },
    {
      text: "If two lines are extended to infinity in both directions and never meet, what are they called?",
      options: [
        "Intersecting Lines",
        "Rays",
        "Parallel Lines",
      ...
      correct: 2,
      feedback: "Correct! Lines that run alongside each other at an equal distance and never cross, even at infinity, are parallel lines."
    }
  ];
    ...
    <div style={{ maxWidth: '650px', margin: '2rem auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {!quizFinished ? (
        ...
        <div className="glass-panel" style={{ padding: '3rem 2rem', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', background: '#090d16', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'rgba(139, 92, 246, 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a78bfa', marginBottom: '0.5rem' }}>
            <Trophy size={36} />
          </div>
          ...
          <p style={{ margin: 0, color: '#94a3b8', fontSize: '0.9rem', lineHeight: '1.6', maxWidth: '400px' }}>
            You scored <strong>{score} out of {questions.length}</strong>! You have mastered the differences between Intersecting and Parallel lines.
          </p>
