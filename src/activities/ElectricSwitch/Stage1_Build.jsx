...
export default function Stage1_Build({ onComplete }) {
  ...
  const handleTerminalClick = (terminalId) => {
    setError('');
    if (!pinsValid) {
      setError("⚠️ Scientific Check Warning: Both drawing pins must be placed on the Cardboard Base! The cardboard sheet acts as an insulator preventing current from leaking into the table/surface.");
      return;
    }
    if (!selectedTerminal) {
      ...
      if (wireId) {
        ...
        if (t1.startsWith('battery') && t2.startsWith('battery')) {
          setError("❌ Short Circuit! Connecting the positive and negative terminals of a battery directly causes a short circuit.");
        } else if (t1.startsWith('bulb') && t2.startsWith('bulb')) {
          setError("❌ Connecting the bulb terminals together won't power the bulb. You need to connect it to the battery and switch.");
        } else if (t1.startsWith('pin') && t2.startsWith('pin')) {
          setError("❌ Connecting the switch pins directly bypasses the circuit loop.");
        } else {
          setError("❌ Invalid connection. Follow the dotted guide lines to connect the circuit!");
        }
      }
      setSelectedTerminal(null);
    }
  ...
  const physicalSteps = ['cardboard', 'pin1', 'safetyPin', 'pin2', 'battery', 'bulb'];
  ...
  const getNextStepPrompt = () => {
    ...
    if (placedCount === 0) return '🔬 Choose any component from the tray below and drag it onto the canvas to begin!';
    const remaining = STEPS.filter(s => s.id !== 'wires' && !placed[s.id]);
    if (remaining.length > 0) return `🔧 ${remaining.length} component${remaining.length > 1 ? 's' : ''} left — pick any from the tray and place them on the canvas.`;
      ...
      <div className="main-grid" style={{ gridTemplateColumns: '1fr', gap: '1rem', padding: '1rem', maxWidth: '850px', margin: '0 auto' }}>
        ...
        <CanvasDroppable>
          ...
          >
            ...
            {!placed.cardboard && (
              <rect x={370} y={200} width={160} height={210} rx={12} fill="none" stroke="#6366f1" strokeWidth={1.5} strokeDasharray="4,4" opacity={0.3} />
            )}
            
            {/* Bulb Dotted outline */}
            {placed.cardboard && !placed.bulb && (
              <g opacity={0.3}>
                <rect x={260} y={80} width={80} height={20} rx={4} fill="none" stroke="#6366f1" strokeWidth={1.5} strokeDasharray="3,3" />
            ...
            {placed.cardboard && !placed.battery && (
              <rect x={104} y={366} width={92} height={48} rx={6} fill="none" stroke="#6366f1" strokeWidth={1.5} strokeDasharray="4,4" opacity={0.3} />
            )}

            {/* Pin 1 Dotted outline */}
            {!placed.pin1 && (
              <circle cx={450} cy={250} r={14} fill="none" stroke="#6366f1" strokeWidth={1.5} strokeDasharray="3,3" opacity={0.3} />
            )}

            {/* Safety Pin Dotted outline */}
            {!placed.safetyPin && (
              <g transform="translate(450, 250) rotate(-35)" opacity={0.3}>
                <circle cx={0} cy={0} r={8} fill="none" stroke="#6366f1" strokeWidth={1.5} strokeDasharray="3,3" />
                <line x1={0} y1={0} x2={0} y2={110} stroke="#6366f1" strokeWidth={1.5} strokeDasharray="3,3" />
              </g>
            )}

            {/* Pin 2 Dotted outline */}
            {!placed.pin2 && (
              <circle cx={450} cy={370} r={14} fill="none" stroke="#6366f1" strokeWidth={1.5} strokeDasharray="3,3" opacity={0.3} />
            )}
            ...
            {placed.battery && placed.bulb && !connectedWires.includes('wire1') && (
              <path 
                d={getWirePath(getTerminalCoords('battery-neg'), getTerminalCoords('bulb-left'))} 
                fill="none" 
                stroke="#6366f1" 
                strokeWidth={2.5} 
                strokeDasharray="4,4" 
            ...
            {placed.battery && placed.pin2 && !connectedWires.includes('wire2') && (
              <path 
                d={getWirePath(getTerminalCoords('battery-pos'), getTerminalCoords('pin2'))} 
                fill="none" 
                stroke="#6366f1" 
                strokeWidth={2.5} 
                strokeDasharray="4,4" 
            ...
            {placed.pin1 && placed.bulb && !connectedWires.includes('wire3') && (
              <path 
                d={getWirePath(getTerminalCoords('pin1'), getTerminalCoords('bulb-right'))} 
                fill="none" 
                stroke="#6366f1" 
                strokeWidth={2.5} 
                strokeDasharray="4,4" 
            ...
            {connectedWires.includes('wire1') && (
              <>
                ...
                <path d={getWirePath(getTerminalCoords('battery-neg'), getTerminalCoords('bulb-left'))} fill="none" stroke="#ef4444" strokeWidth={2.5} strokeLinecap="round" />
              </>
            )}
            {/* Wire 2 */}
            ...
            {placed.pin1 && (
              <DraggableSVGGroup id="pin1" isDraggable={!success}>
                ...
                {placed.safetyPin && <SafetyPinSVG x={positions.pin1.x} y={positions.pin1.y} rotation={-35} isPlaced={true} />}
              </DraggableSVGGroup>
            )}
            ...
            {isDragging && activeStep && (
              <g className="pulse-target" style={{ opacity: 0.8 }}>
                {activeStep.id === 'cardboard' && (
                  <rect x={370} y={200} width={160} height={210} rx={12} fill="rgba(99, 102, 241, 0.05)" stroke="#6366f1" strokeWidth={2.5} strokeDasharray="5,5" />
                )}
                {activeStep.id === 'pin1' && (
                  <circle cx={450} cy={250} r={14} fill="rgba(99, 102, 241, 0.05)" stroke="#6366f1" strokeWidth={2.5} strokeDasharray="3,3" />
                )}
                {activeStep.id === 'safetyPin' && (
                  <g transform="translate(450, 250) rotate(-35)">
                    <circle cx={0} cy={0} r={10} fill="none" stroke="#6366f1" strokeWidth={2} strokeDasharray="3,3" />
                    <line x1={0} y1={0} x2={0} y2={110} stroke="#6366f1" strokeWidth={2} strokeDasharray="3,3" />
                  </g>
                )}
                {activeStep.id === 'pin2' && (
                  <circle cx={450} cy={370} r={14} fill="rgba(99, 102, 241, 0.05)" stroke="#6366f1" strokeWidth={2.5} strokeDasharray="3,3" />
                )}
                {activeStep.id === 'battery' && (
                  <rect x={104} y={366} width={92} height={48} rx={6} fill="rgba(99, 102, 241, 0.05)" stroke="#6366f1" strokeWidth={2.5} strokeDasharray="5,5" />
                )}
                {activeStep.id === 'bulb' && (
                  <g>
                    <rect x={260} y={80} width={80} height={20} rx={4} fill="rgba(99, 102, 241, 0.05)" stroke="#6366f1" strokeWidth={2.5} strokeDasharray="3,3" />
                    <circle cx={300} cy={45} r={22} fill="none" stroke="#6366f1" strokeWidth={2.5} strokeDasharray="3,3" />
                  </g>
                )}
              </g>
            )}
            ...
            {selectedItemId === 'wires' && terminals.map(t => {
              ...
              const strokeColor = !pinsValid ? '#64748b' : (isSelected ? '#60a5fa' : t.color);
              const fillColor = !pinsValid ? '#94a3b8' : (isSelected ? '#3b82f6' : (isConnected ? '#10b981' : '#ffffff'));
              ...
              return (
                ...
                >
                  <circle 
                    ...
                    style={{ opacity: pinsValid ? 1 : 0.6, transition: 'all 0.2s' }}
                  />
                  <circle 
                    r={4} 
                    fill={fillColor} 
          ...
          <div style={{ position: 'absolute', bottom: '0.75rem', left: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.15rem', pointerEvents: 'none' }}>
            ...
            <span style={{ fontSize: '0.7rem', color: '#64748b' }}>
              {selectedItemId === 'wires' 
                ? (selectedTerminal ? `Connecting from: ${terminals.find(t => t.id === selectedTerminal)?.label}` : "Wires selected: Click terminal dots to connect")
                : (activeStep ? `Ready to place: ${activeStep.name}` : "Workspace awaits components...")}
            </span>
          </div>
            ...
            {selectedItemId === 'wires' ? (
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '0.5rem' }}>
                  <h4 style={{ margin: 0, fontSize: '1rem', color: '#ffffff' }}>Connecting Wires</h4>
                  {!pinsValid ? (
                    <div style={{ 
                      ...
                      <strong style={{ fontSize: '0.75rem', color: '#f87171', display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
                        ⚠️ Scientific Check Warning
                      </strong>
                      <p style={{ margin: '0.3rem 0 0 0', fontSize: '0.7rem', color: '#cbd5e1', lineHeight: '1.3' }}>
                        Both drawing pins must be placed on the Cardboard Base. 
                        <strong>Why?</strong> Cardboard is an <strong>insulator</strong> (non-conductor). Placing drawing pins directly on the workspace would leak electric current if the underlying table is conductive. The cardboard sheet provides a safe insulating base.
                      </p>
                      <p style={{ margin: '0.4rem 0 0 0', fontSize: '0.68rem', color: '#f87171', fontWeight: 'bold' }}>
                        👉 Drag the drawing pins or the cardboard base on the canvas to correct their positions.
                      </p>
                    </div>
                  ) : (
                    <>
                      ...
                      <div style={{ fontSize: '0.75rem', color: '#818cf8', fontWeight: 'bold', marginTop: '0.5rem' }}>
        ...
        <div className="glass-panel" style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start', background: 'rgba(30, 41, 59, 0.4)', padding: '0.6rem 0.8rem', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.04)' }}>
