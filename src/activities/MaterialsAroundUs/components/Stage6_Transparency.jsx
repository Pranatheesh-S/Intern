      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '1.5rem' }}>
        ...
        <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', border: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', flex: 1, minHeight: '180px' }}>
            {hidingSpots.map((spot) => (
              ...
              >
                {spot.id === 'wall' ? (
                  <EyeOff size={32} style={{ color: 'var(--danger)' }} />
                ) : spot.id === 'frosted' ? (
                  <ShieldAlert size={32} style={{ color: 'var(--warning)' }} />
                ) : (
                  <Eye size={32} style={{ color: 'var(--success)' }} />
                )}
                <span style={{ fontWeight: 'bold', fontSize: '0.85rem' }}>{spot.name}</span>
                <span style={{ fontSize: '0.65rem', background: 'var(--card-bg)', padding: '0.1rem 0.4rem', borderRadius: '4px', border: '1px solid var(--border)' }}>
                  {spot.type}
                </span>
              </button>
            ))}
          ...
          {/* Dialogue display */}
          <div style={{ minHeight: '80px', background: 'var(--surface)', padding: '0.75rem', borderRadius: '8px', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
            {selectedHidingSpot ? (
              <>
                <strong style={{ fontSize: '0.8rem', color: 'var(--text-heading)' }}>
                  Spot Type: {selectedHidingSpot.type}
                </strong>
                <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', margin: 0, lineHeight: '1.4' }}>
                  {selectedHidingSpot.seekerDialog} <br/>
                  <span style={{ color: 'var(--accent)', fontWeight: 'bold' }}>{selectedHidingSpot.description}</span>
                </p>
              </>
            ) : (
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', display: 'block', textAlign: 'center', marginTop: '1rem' }}>
        ...
        <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', border: '1px solid var(--border)', background: 'var(--card-bg)' }}>
          <div style={{ borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem' }}>
            <span style={{ fontWeight: 'bold', fontSize: '0.95rem' }}>2. Activity 6.6 Classifier</span>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 1, overflowY: 'auto' }}>
            {objectsToClassify.map((obj) => {
              ...
              return (
                ...
                >
                  <span style={{ fontSize: '0.8rem', fontWeight: 'bold', color: 'var(--text-heading)' }}>{obj.name}</span>
          ...
          {classificationComplete && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'var(--success)', fontSize: '0.8rem', fontWeight: 'bold', justifyContent: 'center' }}>
              <Check size={16} /> Table 6.4 Completed!
            </div>
          )}
        </div>
      </div>
