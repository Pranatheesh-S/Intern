...
export default function Stage6a_Surveillance({ onComplete, addXp }) {
  ...
  const spots = [
    ...
    },
    {
      ...
      renderScene: () => (
        <div style={{ position: 'relative', width: '100%', height: '100%', background: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {/* Room Background */}
          ...
          <div style={{ position: 'absolute', right: '15px', top: '50%', width: '10px', height: '40px', background: '#FFFFFF', borderRadius: '4px', zIndex: 11 }} />
        </div>
      )
    },
    {
      ...
      renderScene: () => (
        <div style={{ position: 'relative', width: '100%', height: '100%', background: '#FFFFFF', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', overflow: 'hidden' }}>
          ...
          {/* Leaves */}
          <div style={{ position: 'absolute', top: '-20px', width: '200px', height: '100px', background: '#FFFFFF', borderRadius: '50%', zIndex: 11, filter: 'blur(5px)' }} />
        </div>
      )
    },
    {
      ...
      renderScene: () => (
        <div style={{ position: 'relative', width: '100%', height: '100%', background: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {/* Window mullions */}
          <div style={{ position: 'absolute', top: 0, bottom: 0, left: '50%', width: '8px', background: '#FFFFFF', zIndex: 11 }} />
          <div style={{ position: 'absolute', left: 0, right: 0, top: '50%', height: '8px', background: '#FFFFFF', zIndex: 11 }} />
        </div>
      )
    }
  ];
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%', height: '100%', color: '#1e293b' }}>
      ...
      {/* Header */}
      <div style={{ background: '#FFFFFF', border: '1px solid #D9C9A3', borderRadius: '16px', padding: '1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          <h3 style={{ margin: 0, fontSize: '1.4rem', color: '#3B2A1F', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 2, background: '#FFFFFF', borderRadius: '16px', border: '4px solid #D9C9A3', overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr', position: 'relative' }}>
          ...
          {spots.map((spot, index) => (
            ...
            >
              ...
              <div style={{ position: 'absolute', top: '15px', left: '15px', background: 'rgba(255, 255, 255, 0.9)', border: '1px solid #D9C9A3', color: '#1e293b', padding: '6px 12px', borderRadius: '20px', fontSize: '0.85rem', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px', zIndex: 20 }}>
                <div style={{ background: '#FFFFFF', width: '20px', height: '20px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>{index + 1}</div>
                {spot.name}
              </div>

              <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: 'rgba(255, 255, 255, 0.5)', backdropFilter: 'blur(4px)', border: '2px solid #D9C9A3', borderRadius: '50%', width: '60px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 20, pointerEvents: 'none', boxShadow: '0 0 15px rgba(59,130,246,0.3)' }}>
                <Search size={30} color="#3b82f6" />
              </div>

              {observations[spot.id] && (
                <div style={{ position: 'absolute', bottom: '15px', right: '15px', background: '#FFFFFF', color: 'white', padding: '4px 8px', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 'bold', zIndex: 20 }}>
                  Observed
                </div>
              )}
            </motion.div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #D9C9A3', padding: '1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            <AnimatePresence mode="wait">
              {selectedSpot ? (
                ...
                >
                  <div style={{ background: '#FFFFFF', borderRadius: '12px', padding: '1rem', border: '1px solid #D9C9A3', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  ...
                  <div style={{ background: '#FFFFFF', borderRadius: '12px', padding: '1rem', border: '1px solid #D9C9A3', marginTop: 'auto' }}>
                    <div style={{ color: '#2563eb', fontSize: '0.85rem', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Conclusion</div>
                    <div style={{ color: '#3B2A1F', fontSize: '1rem', lineHeight: '1.5', fontWeight: 'bold' }}>
          ...
          <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #D9C9A3', padding: '1.25rem' }}>
      ...
      {/* Footer Progress */}
      <div style={{ background: '#ffffff', border: '1px solid #D9C9A3', borderRadius: '12px', padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {selectedSpot && (
          ...
          >
            <motion.div
              ...
              style={{ width: '80%', height: '80%', borderRadius: '24px', overflow: 'hidden', position: 'relative', border: '4px solid #D9C9A3', boxShadow: '0 0 50px rgba(59, 130, 246, 0.5)' }}
            ...
            >
              ...
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.9), transparent)', padding: '40px 20px 20px', zIndex: 50, color: 'white', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <button 
                  onClick={() => setSelectedSpot(null)}
                  style={{ alignSelf: 'flex-start', background: '#FFFFFF', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '8px', fontWeight: 'bold', marginTop: '10px', cursor: 'pointer' }}
                >
                  Close Observation
                </button>
              </div>
