...
export default function InvestigationHandbook({ highestUnlockedIndex = 0, currentFlowIndex = 0, stageCompleted = false, fullPageMode = false }) {
  ...
  return (
    <div style={{
      ...
      background: fullPageMode ? 'transparent' : 'white', borderRadius: fullPageMode ? '0' : '8px',
      boxShadow: fullPageMode ? 'none' : '0 20px 50px rgba(0,0,0,0.3)',
      display: 'flex', flexDirection: 'column',
      border: fullPageMode ? 'none' : 'clamp(6px, 1.5vw, 18px) solid #1b2a4a',
      position: 'relative',
      fontFamily: 'Arial, Helvetica, sans-serif',
      overflow: 'hidden'
    }}>
      {/* Book Spine (Right Edge) */}
      {!fullPageMode && <div style={{ position: 'absolute', top: 0, bottom: 0, right: 0, width: '30px', background: 'linear-gradient(to right, transparent, rgba(0,0,0,0.1))', pointerEvents: 'none', zIndex: 10 }} />}
      {!fullPageMode && <div style={{ position: 'absolute', top: 0, bottom: 0, right: 0, width: '2px', background: 'rgba(0,0,0,0.1)', zIndex: 10 }} />}
      ...
      {!isBarrier2 && !isBarrier3 ? (
        fullPageMode ? (
          <div style={{ flex: 1, minHeight: 0, padding: '0px 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px', overflow: 'hidden' }}>
            ...
            <div style={{ minHeight: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', borderLeft: '1px dashed #cbd5e1', paddingLeft: '32px' }}>
              <h2 style={{ margin: '0 0 16px 0', fontSize: 'var(--text-2xl)', color: '#1b2a4a', fontWeight: 'bold', borderBottom: '4px solid #3b4ea0', paddingBottom: '8px', display: 'inline-block' }}>
