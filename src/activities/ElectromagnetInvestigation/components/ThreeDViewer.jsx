...
function CoilModel() {
  return (
    <group scale={2} rotation={[0, 0, Math.PI / 2]}>
...
function ChartPaperModel() {
  return (
    <group rotation={[0, 0, Math.PI / 2]} scale={1.5}>
      <Cylinder args={[0.2, 0.2, 3, 32]} position={[0, 0, 0]}>
...
function CompassModel() {
  return (
    <group scale={1.5}>
      ...
      {/* Needle */}
      <group position={[0, 0.12, 0]} rotation={[0, Math.PI / 4, 0]}>
...
function WiresModel() {
  const points = [[-1.0, -0.3, 0.2], [-0.85, -0.25, 0.15], [-0.6, -0.05, 0.05], [-0.3, 0.15, -0.05], [0.0, 0.2, -0.1], [0.3, 0.1, -0.05], [0.6, -0.1, 0.05], [0.8, -0.22, 0.1], [0.9, -0.25, 0.12], [1.0, -0.3, 0.15]];
  return (
    <group rotation={[0.1, 0.2, 0]} scale={1.5}>
      {points.map((p, i) => {
        if (i === points.length - 1) return null;
        const isCopper = i === 0 || i === points.length - 2;
        return <WireSegment key={i} p1={p} p2={points[i + 1]} radius={isCopper ? 0.025 : 0.045} color={isCopper ? '#ca8a04' : 'var(--danger)'} />;
      })}
    </group>
  );
}
...
export default function ThreeDViewer({ componentId }) {
  const getModel = () => {
    switch (componentId) {
      case 'chart_paper': return <ChartPaperModel />;
