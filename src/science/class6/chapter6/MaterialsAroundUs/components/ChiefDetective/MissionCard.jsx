...
export default function MissionCard({ mission, onAccept }) {
  const renderStars = (diff) => {
    return Array(3).fill(0).map((_, i) => (
      <span key={i} style={{ color: i < diff ? 'var(--lesson-warning)' : 'var(--lesson-secondary)' }}>★</span>
    ));
  };
  ...
  return (
    <div className="fade-slide-up" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1 }}>
        <p style={{ fontSize: '1.05rem', color: 'var(--lesson-secondary)', lineHeight: '1.6', margin: '0 0 1.5rem 0' }}>
          {mission.dialogue}
        </p>
        ...
        <div style={{ background: 'var(--lesson-surface)', border: '1px solid var(--lesson-border)', borderRadius: '8px', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
            ...
            <div>
              <strong style={{ fontSize: '0.85rem', color: 'var(--lesson-muted)', display: 'block', textTransform: 'uppercase' }}>Objective</strong>
              <span style={{ fontSize: '1rem', color: 'var(--lesson-text)' }}>{mission.objective}</span>
            </div>
          </div>
          
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '0.5rem', paddingTop: '0.75rem', borderTop: '1px dashed var(--lesson-border)' }}>
            <div>
              <strong style={{ fontSize: '0.85rem', color: 'var(--lesson-muted)', display: 'block', textTransform: 'uppercase' }}>Difficulty</strong>
              <div style={{ fontSize: '1.1rem', letterSpacing: '2px' }}>{renderStars(mission.difficulty)}</div>
            </div>
            <div>
              <strong style={{ fontSize: '0.85rem', color: 'var(--lesson-muted)', display: 'block', textTransform: 'uppercase' }}>Est. Time</strong>
              <span style={{ fontSize: '0.95rem', color: 'var(--lesson-text)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
