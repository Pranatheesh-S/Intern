...
export default function Handbook_Hardness({ highestUnlockedIndex = 0, currentFlowIndex = 0, stageCompleted = false }) {
  ...
  return (
    <div style={{
        ...
        border: 'clamp(6px, 1.5vw, 18px) solid var(--text-heading)', // Dark blue border
        position: 'relative',
        overflow: 'hidden'
    }}>
      {/* Notebook spine */}
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: '20px', background: 'linear-gradient(to right, var(--text-muted), var(--border))', borderRight: '1px solid var(--text-muted)', zIndex: 10 }}>
        {[...Array(12)].map((_, i) => (
      ...
      {bookPage === 1 ? (
        <div style={{ flex: 1, minHeight: 0, padding: '24px 20px', paddingLeft: '32px', position: 'relative', display: 'flex', flexDirection: 'column', overflowY: 'auto', gap: '16px' }}>
          ...
          <div style={{ fontSize: 'var(--text-base)', color: 'var(--text-primary)', lineHeight: '1.6', marginBottom: '10px' }}>
          ...
          <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '12px', padding: '16px', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
            ...
            <div style={{ fontSize: 'var(--text-base)', color: 'var(--text-primary)', lineHeight: '1.6', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        <div style={{ flex: 1, minHeight: 0, padding: '24px 20px', paddingLeft: '32px', position: 'relative', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
          ...
          <h2 style={{ margin: '0 0 16px 0', fontSize: 'var(--text-xl)', color: 'var(--text-heading)', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Target size={24} style={{ color: 'var(--accent)' }} /> Investigation: Hardness
          </h2>
          ...
          <div style={{ border: '2px solid #22c55e', borderRadius: '12px', padding: '16px', background: '#f0fdf4', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: 'var(--text-sm)', color: '#166534' }}>
                {isPhase1Done ? <CheckSquare size={18} color="#16a34a" /> : <Square size={18} color="var(--text-muted)" />} Phase 1: Press Test
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: 'var(--text-sm)', color: '#166534' }}>
                {isPhase2Done ? <CheckSquare size={18} color="#16a34a" /> : <Square size={18} color="var(--text-muted)" />} Phase 2: Material ID Test
              </div>
            </div>
          </div>
          ...
          <div style={{ border: '1px solid var(--border)', borderRadius: '12px', padding: '16px', background: 'var(--surface)', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: 'var(--text-sm)', color: 'var(--text-primary)' }}>
                <div style={{ background: 'var(--surface)', padding: '6px', borderRadius: '8px', display: 'flex', color: 'var(--accent)' }}><MousePointer2 size={16} /></div> Click an object from the list.
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: 'var(--text-sm)', color: 'var(--text-primary)' }}>
                <div style={{ background: 'var(--surface)', padding: '6px', borderRadius: '8px', display: 'flex', color: 'var(--accent)' }}><Layers size={16} /></div> Choose the material it is made of.
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: 'var(--text-sm)', color: 'var(--text-primary)' }}>
                <div style={{ background: 'var(--surface)', padding: '6px', borderRadius: '8px', display: 'flex', color: 'var(--accent)' }}><Lightbulb size={16} /></div> It will be added to your Evidence Board.
              </div>
            </div>
          </div>
